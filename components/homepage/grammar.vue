          ...
          <div>
            <div class="relative aspect-video w-[1400px] h-[600px]">
              <form @submit.prevent="handleSubmit" @keydown.enter.prevent="handleSubmit">
                ...
                <div v-if="showModal" class="flex items-center justify-center z-50" id="1">
                  <div class="bg-white p-6 rounded shadow-md absolute left-1 top-12">
                    <p>Did you mean: <strong>{{ currentSuggestion }}</strong>?</p>
                    <button @click="applySuggestion" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded bg-[#753fea] hover:bg-[#5424b3]">
                      Apply
                    </button>
                ...
                <div class="flex flex-row justify-between items-center">
                  ...
                  <UButton @click="handleSave" class="px-16 py-8 text-2xl font-bold absolute bottom-2 left-2 bg-[#753fea] hover:bg-[#5424b3]" :class="isLoading === 'success' ? '' : 'hidden'">
                    Save
                  </UButton>
                </div>
              </form>
